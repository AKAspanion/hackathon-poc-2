...
def _interpolate_waypoints(supplier: str, oem: str, transit_days: int) -> list[str]:
    """
    Build a list of `transit_days` location labels for the shipment timeline.
    Day 1 = supplier, last day = OEM. Middle days are labeled as "In Transit (Day N)".
    For weather we use supplier for first half and OEM city for second half.
    """
    locations = []
    for i in range(transit_days):
        if i == 0:
            ...
            # Use supplier city for first half, oem city for second half of journey
...
def _extract_day_weather_from_forecast(
    ...
    """Extract a specific date's weather from a forecast API response."""
    try:
...
def _extract_day_weather_from_history(
    ...
    """Extract weather from historical API response."""
    try:
...
def _weather_snapshot_to_current_dict(snap: DayWeatherSnapshot) -> dict[str, Any]:
    """Convert a DayWeatherSnapshot back to a current-weather-like dict for risk engine."""
    return {
        "temp_c": snap.temp_c,
...
def _build_risk_analysis_payload(
    ...
    """
    Build a structured payload suitable for consumption by a Risk Analysis Agent.
    """
    ...
    # Aggregate all unique concerns and actions
...
async def run_shipment_weather_agent(
    ...
    # Pre-fetch forecast data for both cities (up to 14 days forward)
    ...
    for i, waypoint_city in enumerate(waypoints):
        ...
        # Determine if this day is historical, current, or future
        ...
        if is_today:
            ...
            # Use forecast — pick the right city's forecast data
            midpoint = transit_days // 2
            ...
            # If not found in forecast (too far ahead), try fetching fresh
            if not weather_snap:
