...
from app.schemas.trend_insight import TrendInsightResponse, TrendInsightRunResponse
...
async def run_trend_insights(
    ...
    - **oem_name**: Override the OEM display name in the insights (default: OEM from JWT).
    - **excel_path**: Override the Excel file path (default: `data/mock_suppliers_demo.xlsx`).
    """
...
def list_trend_insights(
    scope: str | None = Query(None, description="Filter by scope: material | supplier | global"),
    entity_name: str | None = Query(None, description="Partial match on entity name (case-insensitive)"),
    severity: str | None = Query(None, description="Filter by severity: low | medium | high | critical"),
    risk_opportunity: str | None = Query(None, description="Filter by type: risk | opportunity"),
