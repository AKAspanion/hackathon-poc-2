"""Mock shipment tracking data for 5 AWB codes.

The structure is intentionally close to Shiprocket's tracking API shape so that
an LLM agent can reason over realistic fields like shipment_track_activities.
"""
...
# 1) On-time: Chennai -> Bangalore, arrives as expected
...
# 2) Delayed: Mumbai -> Bangalore, arrives 2 days after ETD
AWB_MUM_002 = _base_tracking(
    ...
    delivered_date="2022-07-18 16:00:00",  # expected 12th, actually 18th
    current_status="Delivered",
...
# 3) Stagnant: Delhi -> Bangalore, stuck at Nagpur hub for 48+ hours
AWB_DEL_003 = _base_tracking(
    ...
    activities=[
        ...
        # No scans after 8th -> stagnation risk
        {
...
# 4) Ahead of schedule: Pune -> Bangalore, delivered 1 day early
AWB_PUN_004 = _base_tracking(
    ...
    delivered_date="2022-07-16 18:00:00",  # one day before ETD (17th)
    current_status="Delivered",
...
# 5) Slow velocity: Kolkata -> Bangalore, moving but with large gaps between scans
...
def get_tracking(awb_code: str) -> Optional[Dict[str, Any]]:
