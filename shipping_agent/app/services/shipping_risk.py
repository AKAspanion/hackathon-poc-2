...
from .agent import analyze_shipments_for_supplier
...
def calculate_distance_risk(distance_km: Optional[float]) -> float:
...
def calculate_transit_risk(avg_transit_days: Optional[float]) -> float:
...
_PORT_CONGESTION_TABLE: Dict[str, float] = {
    "chennai port": 0.7,
    "los angeles": 0.8,
    "long beach": 0.8,
    "singapore": 0.5,
...
def calculate_port_congestion_risk(port_used: Optional[str]) -> float:
    if not port_used:
        return 0.4
    key = port_used.lower()
    return _PORT_CONGESTION_TABLE.get(key, 0.5)


def calculate_historical_delay_risk(historical_delay_percentage: Optional[float]) -> float:
    if historical_delay_percentage is None:
        return 0.4
    value = max(0.0, min(historical_delay_percentage / 100.0, 1.0))
    return value
...
def calculate_shipping_risk_heuristic(supplier: Supplier) -> Dict:
    ...
    if mode_risk >= 0.6 and supplier.shipping_mode.lower() == "sea":
...
def _normalize_agent_result(raw: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure the LLM result has all expected keys with sensible defaults."""
...
def calculate_shipping_risk(supplier: Supplier, db: Session) -> Dict[str, Any]:
    """Primary entrypoint used by routers.

    Attempts to use the LLM-powered agent; if it fails (e.g. no API key),
    falls back to the heuristic calculator above.
    """
    ...
    try:
        ...
        # Fall back to deterministic heuristic if the agent is unavailable.
